...
export function AdminDashboardTile({ title, icon, buttonText, onClick, infoText}) {
  return (
    <div className="flex flex-col items-center p-4 h-[20em] rounded-md bg-emerald-200 shadow-md font-mono hover:bg-emerald-100 hover:text-white">
      <FontAwesomeIcon icon={icon} className="text-6xl text-emerald-500 mt-8" />
      
      <h2 className="text-base md:text-l lg:text-xl font-mono font-semibold text-emerald-500 mt-2">
        {title}
      </h2>
      
      <button className="bg-emerald-400 rounded-md p-2 my-4 w-5/6 shadow-lg hover:bg-white hover:text-violet-600" onClick={onClick}>
        {buttonText}
      </button>
